...
namespace Drupal\cn_core\Controller;
 ...
 */
class ViewsAjaxController implements ContainerInjectionInterface {
  ...
  public function ajaxView(Request $request) {
    ...
    if (isset($name) && isset($display_id)) {
      ...
      if ($view && $view->access($display_id)) {
        ...
        if ($name == 'job_board_table') {
          $response->addCommand(new RemoveCommand('.view-job-featured-2'));
        }
        elseif ($name == 'event_board_table') {
          $response->addCommand(new RemoveCommand('.view-events-featured-2'));
        }
        elseif (in_array($name, $podcast_views)) {
          $response->addCommand(new InvokeCommand(NULL, 'reloadPodcastPlayers', ['test']));
        }
        return $response;
      }
      else {
