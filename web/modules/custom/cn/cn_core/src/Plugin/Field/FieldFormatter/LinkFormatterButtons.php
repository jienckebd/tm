 ...
 * @FieldFormatter(
 *   id = "link_buttons",
 *   label = @Translation("Link Buttons"),
 *   field_types = {
 *     "link"
 *   }
 * )
 */
class LinkFormatterButtons extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['button_attribute'] = [
      '#type' => 'details',
      '#title' => t('Button Attributes'),
      '#open' => TRUE,
    ];

    $elements['button_attribute']['class'] = [
      '#type' => 'textfield',
      '#title' => t('Class'),
      '#default_value' => !empty($settings['button_attribute']['class']) ? $settings['button_attribute']['class'] : NULL,
    ];
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $link_title = !empty($settings['link_text']) ? $settings['link_text'] : 'Learn More';

    $button_attributes = !empty($settings['button_attribute']) ? $settings['button_attribute'] : [];

    if (!empty($button_attributes['class'])) {
      $button_attributes['class'] = explode(' ', $button_attributes['class']);
    }

    $button_attributes['class'][] = 'button';
    $button_attributes['class'][] = 'btn';
    $button_attributes['class'][] = 'use-ajax';
    $button_attributes['data-dialog-type'] = 'modal';
  ...
  protected function buildUrl(LinkItemInterface $item) {
    ...
    if (!empty($node)) {
      ...
      if (!$url->isExternal()) {
        $url->setRouteParameter('context_entity_type', $context_entity_type);
        $url->setRouteParameter('context_entity_id', $context_entity_id);
      }
    }
