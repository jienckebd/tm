...
class GridStack extends ConfigEntityBase implements GridStackInterface {
  ...
  /**
   * Returns the amount of grid items.
   */
  public function gridCount() {
    return $this->getOptions('grids') ? count($this->getOptions('grids')) : 0;
  }
    ...
    if (!empty($settings['breakpoints']['sm']['grids'])) {
      foreach ($settings['breakpoints'] as $key => $breakpoint) {
        if (!empty($breakpoint['column']) && !empty($breakpoint['grids']) && is_string($breakpoint['grids'])) {
          $settings['breakpoints'][$key]['grids'] = $this->getBreakpointGrids($key);
        }

        // Specific to LG, grids stored under options as array.
        if ($key == 'lg') {
          $settings['breakpoints'][$key]['grids'] = $this->getGrids(TRUE);
        }
      }
    }
  }
