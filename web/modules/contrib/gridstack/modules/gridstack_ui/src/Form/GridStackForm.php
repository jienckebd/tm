...
class GridStackForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['options']['grids'] = [
      ...
      '#attributes'  => [
        'class' => [
          'form-wrapper--gridstack-grid',
          'form-wrapper--gridstack-grids',
          'visually-hidden',
        ],
      ],
    ];
    ...
    $form['options']['breakpoints'] = [
      ...
      '#header'     => [
        $this->t('Breakpoint'),
        $this->t('Max width (not Min width)'),
        $this->t('Image style'),
        $this->t('Column'),
        $this->t('Grids'),
      ],
  ...
  public function getDummyItems($grids = [], $js_settings = [], $image_style_box = '') {
    ...
    $tags = ['button', 'div', 'span', 'select', 'option'];
    foreach ($grids as $key => $grid) {
      $index = $key + 1;
      $box['box']['#allowed_tags'] = $tags;
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($options_breakpoints as $key => $breakpoints) {
      foreach ($breakpoints as $k => $value) {
        ...
        $xs = ['options', 'breakpoints', 'xs_preview', 'preview'];
        $form_state->unsetValue($xs);

        $lg = ['options', 'breakpoints', 'lg_preview', 'preview'];
        $form_state->unsetValue($lg);
      }
    }
